...
const connectDB = require("./utils/detabase");
...
app.get("/item/:id", async (req, res) => {
  try{
    ...
    return res.status(200).json({message: "アイテム読みとり成功（シングル）", singleItem: singleItem})
    
  }catch{
    return res.status(400).json({message: "アイテム読みとり失敗（シングル）"})
